...
void scanFolder(ItemInfo folderContent, int regularInterval) {
    repositories.getChildren(RepoPathFactory.create(folderContent.getRepoKey(), folderContent.getRelPath())).each { item ->
        if (item.isFolder()) {
            scanFolder(item, regularInterval)
        } else {
